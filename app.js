...
app.get("/admin/api/resources/Email/custom-list", async (req, res) => {
  try {
    const resource = adminJS.findResource("Email");
    // const records = await Email.find();
    const records = await resource.find();
    // const meta = resource.decorate().toJSON(req.adminUser);
    console.log(records);
    res.json({
      data: records.toJSON(),
    });
  } catch (error) {
    console.error("Error fetching Email resource:", error);
